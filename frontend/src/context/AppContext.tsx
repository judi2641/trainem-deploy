		...
		async function loadAppData() {
			try {
				...
				const [workoutsRes, entriesRes, habitsRes, exercisesRes, pixelArtRes] = await Promise.all([
					fetch(`http://localhost:3000/api/workouts/${user!.sub}`, { headers: authHeaders }),
					fetch(`http://localhost:3000/api/entries/${user!.sub}`, { headers: authHeaders }),
					fetch(`http://localhost:3000/api/habits/${user!.sub}`, { headers: authHeaders }),
					fetch(`http://localhost:3000/api/exercises`, { headers: authHeaders }),
					fetch(`http://localhost:3000/api/pixel-art/${user!.sub}`, { headers: authHeaders }),
				]);
